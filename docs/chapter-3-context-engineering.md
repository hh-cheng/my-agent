...
当前 `compressor.ts` 是教学版压缩模块，单元测试和 E2E 会直接验证它的行为。入口 `src/index.ts` 目前在线路上使用的是更轻量的三层防线 `applyDefense`，没有每轮都调用 LLM 摘要。这样做是刻意的：摘要会额外调用一次模型，适合作为“历史很长时的重型压缩”，而三层防线适合作为每轮都可以跑的便宜防护。
...
这三个命令在 `src/index.ts` 的 readline 循环里被拦截，不会写入 `messages`，也不会触发模型调用。
