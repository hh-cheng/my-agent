...
function getModelApiKey() {
  if (config.model.apiKey && !config.model.apiKey.startsWith('${')) {
    return config.model.apiKey
  }

  return config.model.provider === 'deepseek'
...
const modelName = apiKey ? config.model.name : 'Mock Model'

//* === 工具注册：streamText 通过 tools 参数暴露给模型 ===
const toolRegistry = new ToolRegistry()
toolRegistry.setRole(config.security.defaultRole)
...
const budget: BudgetState = { used: 0, limit: config.agents.budgetLimit }
...
registeredPipelines.pre.forEach(({ name, pipeline }) =>
  name !== 'audit-log' || config.security.auditLog
    ? hookPipelines.registerPre(name, pipeline)
    : undefined,
)
registeredPipelines.post.forEach(({ name, pipeline }) =>
  name !== 'bash-timestamp' || config.security.bashTimestamp
...
export async function startAgent() {
  ...
  if (config.channels.feishu.enabled) {
    ...
    loadedPlugins.add(feishuPlugin.name)
  }
  for (const pluginConfig of config.plugins) {
    ...
    loadedPlugins.add(pluginConfig.name)
  }
  await gateway.startAll()

  //* === Cron Service ===
  if (config.cron.enabled) cronService.load()
